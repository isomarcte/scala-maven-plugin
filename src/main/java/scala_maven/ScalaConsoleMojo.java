...
public class ScalaConsoleMojo extends ScalaMojoSupport {
    ...
    protected void doExecute() throws Exception {
        // Force no forking
        final JavaMainCaller jcmd =
            super.getScalaCommand(false,
                                  this.mainConsole);
        // Determine Scala Version
        final VersionNumber scalaVersion =
            super.findScalaVersion();
        final Set<String> classpath =
            this.setupClassPathForConsole(scalaVersion);
        ...
        final String classpathStr =
            MainHelper.toMultiPath(classpath.toArray(new String[classpath.size()]));
    ...
    private void handleJavaRebel(final JavaMainCaller jcmd) throws IOException {
        if (this.javaRebelPath != null) {
            final String canonicalJavaRebelPath =
                this.javaRebelPath.getCanonicalPath();
            if (this.javaRebelPath.exists()) {
                jcmd.addJvmArgs("-noverify",
                                String.format("-javaagent:%s",
                                              canonicalJavaRebelPath));
            } else {
                super.getLog().warn(String.format("javaRevelPath '%s' not found",
                                                  canonicalJavaRebelPath));
            }
        }
    }
    ...
    private Set<String> setupConsoleClasspaths(final VersionNumber scalaVersion) throws Exception {
        ...
        super.addToClasspath(this.resolveJLine(scalaVersion,
                                               this.fallbackJLine(scalaVersion)),
                             classpath,
                             true);
    ...
    private Artifact fallbackJLine(final VersionNumber scalaVersion) {
        ...
        final VersionNumber scala2_12_0M4 =
            new VersionNumber("2.12.0-M4");
        final VersionNumber scala2_11_0 =
            new VersionNumber("2.11.0");
        final VersionNumber scala2_9_0 =
            new VersionNumber("2.9.0");
